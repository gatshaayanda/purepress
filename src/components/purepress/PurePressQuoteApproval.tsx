...
import { formatPurePressDate, formatPurePressMoney } from "@/lib/purepress/i18n";
...
export default function PurePressQuoteApproval({ token }: { token:string }) {
  ...
  return <main className={styles.page}><div className={styles.shell}><Brand/>{error&&<p className={styles.error} role="alert">{error}</p>}
    <header className={styles.hero}><div><span>QUOTATION</span><h1>{quote.quoteNumber}</h1><p>Revision {quote.revision} · Job {quote.job.referenceCode}</p></div><div className={styles.total}><small>TOTAL BWP</small><strong>{formatPurePressMoney(locale, quote.totalMinor)}</strong><span>{stateCopy(quote)}</span></div></header>
    <section className={styles.customer}><div><small>PREPARED FOR</small><strong data-pp-no-translate>{quote.customer.displayName}</strong>{quote.customer.organisation&&<span>{quote.customer.organisation}</span>}</div><div><small>WORK QUOTED</small><strong data-pp-no-translate>{quote.job.itemSummary}</strong>{quote.job.quantity&&<span>{quote.job.quantity} item(s)</span>}</div><div><small>VALID UNTIL</small><strong>{quote.validUntil ? formatPurePressDate(locale, quote.validUntil) : "Not stated"}</strong></div></section>
    <section className={styles.lines} aria-label="Quotation line items"><div className={styles.lineHead}><span>Description</span><span>Qty</span><span>Unit</span><span>Total</span></div>{quote.lineItems.map(line=><div className={styles.line} key={line.id}><span data-pp-no-translate><strong>{line.description}</strong><small>{line.category.replaceAll("_"," ")}</small></span><span>{line.quantity}</span><span>{formatPurePressMoney(locale, line.unitPriceMinor)}</span><span>{formatPurePressMoney(locale, line.lineTotalMinor)}</span></div>)}</section>
    <section className={styles.totals}><span>Subtotal<strong>{formatPurePressMoney(locale, quote.subtotalMinor)}</strong></span>{quote.discountMinor>0&&<span>Discount<strong>{formatPurePressMoney(locale, quote.discountMinor)}</strong></span>}{quote.tax&&<span>{quote.tax.label} ({quote.tax.ratePercent}%)<strong>{formatPurePressMoney(locale, quote.tax.amountMinor)}</strong></span>}<span className={styles.grand}>TOTAL BWP<strong>{formatPurePressMoney(locale, quote.totalMinor)}</strong></span></section>
    <div className={styles.detailGrid}>{quote.job.customerRequestedDate&&<Info title="Customer requested date" text={formatPurePressDate(locale, quote.job.customerRequestedDate)}/>} {quote.job.customerTimingFlexible&&!quote.job.customerRequestedDate&&<Info title="Customer requested timing" text="Flexible"/>}{quote.estimatedCompletionDate&&<Info title="Estimated completion" text={formatPurePressDate(locale, quote.estimatedCompletionDate)}/>} {quote.estimatedTurnaroundText&&<Info title="Estimated turnaround" text={quote.estimatedTurnaroundText}/>} {quote.paymentTerms&&<Info title="Payment terms" text={quote.paymentTerms}/>} {quote.fulfillmentNotes&&<Info title="Collection / delivery" text={quote.fulfillmentNotes}/>} {quote.customerVisibleNotes&&<Info title="Quote notes" text={quote.customerVisibleNotes}/>}</div>
